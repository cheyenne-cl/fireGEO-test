...
export async function performAnalysis({
  ...
  console.log(
    "Available providers for analysis:",
      ...
      availableProviders.map(async (provider) => {
        ...
        try {
          ...
          if (response === null) {
            ...
            // Generate a fallback response for failed providers
            const fallbackResponse = {
              provider: provider.name,
              prompt: prompt.prompt,
              response: `Analysis not available for ${provider.name} due to configuration issues.`,
              rankings: [],
              competitors: [],
            ...
            // Send analysis complete event with failed status
            await sendEvent({
