...
interface AnalysisProgressSectionProps {
  ...
  dispatch: (action: BrandMonitorAction) => void;
}
...
export function AnalysisProgressSection({
  ...
  // Use provided prompts or show loading state
        ...
        <div className="transition-all duration-400 opacity-100 translate-y-0">
          <Card className="p-2 bg-card text-card-foreground gap-6 rounded-xl border py-6 shadow-sm rounded-xl border-2 border-solid border-[#000589] h-full flex flex-col">
            ...
            <CardContent className="space-y-6">
              {/* Prompts tiles */}
              <div>
                {displayPrompts.length === 0 && !analyzing ? (
                  <div className="text-center py-8">
                    <p className="text-gray-600 mb-2">No prompts available</p>
                    <p className="text-gray-500 text-sm">
                      Prompts will be generated automatically during analysis
                    </p>
                  </div>
                ) : (
